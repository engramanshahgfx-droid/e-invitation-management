...
function parseCSV(content: string): any[] {
  ...
  const nameIdx = headers.findIndex(
    (h) => h === 'name' || h === 'guestname' || h === 'fullname' || h === 'firstname' || h === 'guest'
  )

  const phoneIdx = headers.findIndex((h) => h.includes('phone') || h === 'tel' || h === 'telephone' || h === 'contact')

  const emailIdx = headers.findIndex((h) => h === 'email' || h === 'emailaddress' || h === 'e_mail' || h === 'mail')
  ...
  const plusOnesIdx = headers.findIndex(
    (h) =>
      h.includes('plusone') ||
      h.includes('plus1') ||
      h.includes('guests') ||
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (values.length <= Math.max(nameIdx, phoneIdx, emailIdx)) {
      console.log(
        `Skipping incomplete row ${i}: expected ${Math.max(nameIdx, phoneIdx, emailIdx) + 1} columns, got ${values.length}`
      )
      continue
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (checkError) {
      ...
      return NextResponse.json({ error: 'Failed to check for duplicate guests' }, { status: 400 })
    }
    ...
    if (replaceExisting) {
      ...
      if (deleteError) {
        ...
        return NextResponse.json({ error: 'Failed to delete existing guests' }, { status: 400 })
      }
    }
    ...
    const { data: insertedGuests, error: insertError } = await supabase.from('guests').insert(guests).select()

    if (insertError) {
      console.error('Database insert error:', insertError)
      return NextResponse.json({ error: insertError.message || 'Failed to save guests' }, { status: 400 })
    }
