...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: existingGuests } = await supabase.from('guests').select('phone').eq('event_id', eventId)
    ...
    if (insertError) {
      ...
      return NextResponse.json({ error: `Failed to create guest: ${insertError.message}` }, { status: 500 })
    }
