...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!adminId || !planId || !updates) {
      return NextResponse.json({ error: 'adminId, planId, and updates are required' }, { status: 400 })
    }
    ...
    if (updateError) {
      return NextResponse.json({ error: updateError.message || 'Failed to update plan' }, { status: 400 })
    }
